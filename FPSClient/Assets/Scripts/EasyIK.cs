...
public class EasyIK : MonoBehaviour {
    [Header("IK properties")] public int numberOfJoints = 2;
    ...
    [Header("Debug")] public bool debugJoints = true;
    public bool localRotationAxis = false;

    // Remove this if you need bigger gizmos:
    [Range(0.0f, 1.0f)] public float gizmoSize = 0.05f;
    ...
    void PoleConstraint() {
        if (poleTarget != null && numberOfJoints < 4) {
            int last = jointPositions.Length - 1; // ← dynamic, works with 2 or 3 joints

            var limbAxis = (jointPositions[last] - jointPositions[0]).normalized;
    ...
    private void SolveIK() {
        ...
        for (int i = 0; i < jointPositions.Length - 1; i += 1) {
            ...
            var targetRotation =
                Quaternion.FromToRotation(jointStartDirection[i], jointPositions[i + 1] - jointPositions[i]);
    ...
    void drawHandle(Transform debugJoint) {
        ...
        Handles.ArrowHandleCap(0, debugJoint.position, debugJoint.rotation * Quaternion.LookRotation(Vector3.right),
            gizmoSize, EventType.Repaint);

        Handles.color = Handles.yAxisColor;
        Handles.ArrowHandleCap(0, debugJoint.position, debugJoint.rotation * Quaternion.LookRotation(Vector3.up),
            gizmoSize, EventType.Repaint);

        Handles.color = Handles.zAxisColor;
        Handles.ArrowHandleCap(0, debugJoint.position, debugJoint.rotation * Quaternion.LookRotation(Vector3.forward),
            gizmoSize, EventType.Repaint);
    }
